 ...
 *              $cdata      - Adds or don't CDATA tag to enclose data.
 *
...
function array2xml($array, $name = 'array', $standalone = TRUE, $beginning = TRUE, $cdata = TRUE) {
    global $nested;
    $output = '';
    if ($beginning) {
        if ($standalone) {
    ...
    foreach ($array as $root => $child) {
        if (is_array($child)) {
            ...
            $nested++;
            $output .= array2xml($child, NULL, NULL, FALSE, $cdata);
            $nested--;
            $output .= str_repeat(" ", (2 * $nested)) . '  </' . (is_string($root) ? $root : $ArrayNumberPrefix . $root) . '>' . "\r\n";
        } else {
            $output .= str_repeat(" ", (2 * $nested)) . '  <' . (is_string($root) ? $root : $ArrayNumberPrefix . $root) . '>';
